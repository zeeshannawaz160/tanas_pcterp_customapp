    ...
    const columns = [
        {
            headerName: ' ', field: 'id', sortable: false, filter: false, cellRendererFramework: (params) =>
                <>
                    <Button style={{ minWidth: "4rem", position: 'absolute', top: '50%', transform: 'translateY(-50%)' }} size="sm" as={Link} to={`/${rootPath}/inventoryadjustment/edit/${params.value}`}><BsBoxArrowInUpRight /></Button>
                    {/* <Button style={{ minWidth: "4rem" }} size="sm" as={Link} to={`/employees/employee/${params.value}?mode=view`}><BsEyeFill /></Button> */}
                </>
        },
        { headerName: 'ADJUSTMENT #', field: 'name' },
        { headerName: 'DATE', field: 'date', valueGetter: (params) => params.data?.date ? moment(params.data?.date).format("MM/DD/YYYY ") : "Not Available" },
        { headerName: 'ADJUSTMENT ACCOUNT', field: `inventoryAdjustmentAccount`, valueGetter: (params) => params.data?.inventoryAdjustmentAccount ? params.data?.inventoryAdjustmentAccount[0]?.name : "Not Available" },
        ...
        { headerName: 'ESTIMATED TOTAL', field: 'total', valueGetter: (params) => formatNumber(params.data?.total) },
    ]
                    ...
                    <Row>
                        <Col className='p-0 ps-2'>
                            <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                <Breadcrumb.Item active> <div className='breadcrum-label-active'>INVENTORY ADJUSTMENTS</div></Breadcrumb.Item>
                            </Breadcrumb>
                        </Col>
                    </Row>
                    <Row>
                        <div className="buttonGroup" style={{ height: '100%', display: 'flex', flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
                            <span style={{ display: 'flex', flexDirection: 'row', marginLeft: '-7px' }}>
                                <div className="buttonGroup__back"><Button as={Link} to={`/${rootPath}/inventoryadjustment/add`} variant="primary" size="sm">CREATE</Button></div>
                            </span>
