...
import { Breadcrumb, ButtonGroup, Col, Container, Dropdown, DropdownButton, Form, Row } from 'react-bootstrap';
...
export default function Dashboard() {
    ...
    const handleSelectOption = async (name, duration) => {
        ...
        let response;
        if (rootPath == "purchase") {
            response = await ApiService.get(`purchaseOrder/purchaseAnalysis?name=${name}&start=${startTime}&end=${endTime}`);
        } else {
            response = await ApiService.get(`salesOrder/salesAnalysis?name=${name}&start=${startTime}&end=${endTime}`);
        }
                    ...
                    <Container fluid >
                        <Row>
                            <Col className='p-0 ps-2'>
                                <Breadcrumb style={{ fontSize: '24px', marginBottom: '0 !important' }}>
                                    <Breadcrumb.Item active> <div className='breadcrum-label'>{rootPath == "purchase" ? "PURCHASE" : "SALES"} ANALYSIS</div></Breadcrumb.Item>
                        ...
                        <Row >
                            <Col className='p-0 ps-1'>
                                <Form.Select size='sm' className='reporting-select' id="reportName" name="reportName" aria-label="Default select example" onChange={e => {
                                ...
                                <Form.Select size='sm' className='reporting-select' id="reportDuration" name="reportDuration" aria-label="Default select example" onChange={e => {
