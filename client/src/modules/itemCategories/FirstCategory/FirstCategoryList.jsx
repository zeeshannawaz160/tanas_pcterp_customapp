...
export const FirstCategoryList = () => {
    ...
    useEffect(() => {
        const getAllFirstCategory = async () => {
            await ApiService.get('/itemCategory/search?type=firstCategory')
                .then(response => {
                    if (response.data.isSuccess) {
                        setFirstCategoryList(response.data.document)
                    }
                })
        }
        getAllFirstCategory()
            ...
            <Container className="pct-app-content-container p-0 m-0" fluid>
                <Container className="pct-app-content" fluid>
                    <Container className="pct-app-content-header p-0 m-0 mt-2 pb-2" fluid>
                        ...
                        <Row>
                            <Col><Button as={Link} to={`/itemcategory/firstcategory`} variant="primary" size="sm">Create</Button></Col>
                        </Row>
                    </Container>
                    <Container className="pct-app-content-body p-0 m-0" fluid>
                        <Table striped bordered hover size="sm">
                            ...
                            <tbody>
                                {
                                    firstCategoryList && firstCategoryList.map((element, index) => {
                                        return <tr id={element.id} key={index} onClick={(e) => { }}>
                                            <td >
                                                <Button style={{ minWidth: "4rem" }} as={Link} to={`/itemcategory/firstcategory/${element.id}`} size="sm">Edit</Button>
                                            </td>
