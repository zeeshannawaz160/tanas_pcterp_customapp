...
export default function Payment() {
    ...
    const handleValidateBtn = async () => {
        ...
        if (validateButtonType === 'primary') {
            ...
            estimation.cgst = ((parseFloat(contextValues.total) / 100.00) * 2.5).toFixed(2);
            estimation.sgst = ((parseFloat(contextValues.total) / 100.00) * 2.5).toFixed(2);
            estimation.gst = ((parseFloat(contextValues.total) / 100.00) * 5.0).toFixed(2);
    ...
    const handleReceipt = (data) => {
        ...
        newDoc.setFontSize(6);
        newDoc.setFont("helvetica", "normal");
        newDoc.text("Sale ID: ", 3.5, 23, { align: 'left' });
        newDoc.text(`${data.cashSaleId}`, 13, 22.86, { align: 'left' });
        newDoc.text("Employee ID: ", 3.5, 26, { align: 'left' });
        newDoc.text(`${user.name}`, 18, 26, { align: 'left' });
        newDoc.text("GSTIN: ", 3.5, 29, { align: 'left' });
        newDoc.text(`${user?.gstin}`, 13, 29, { align: 'left' });
        ...
        <Container className="pct-app-content-body p-0 m-0" fluid>
            <div className="PCTAppContent">
                {paymentStatus === 'success' ? <Container className="m-0 pb-2" fluid style={{
                    ...
                    <Row>
                        <Col style={{ textAlign: 'center' }}>
                            <Button className="button" as={Link} to="/pos" style={{ padding: '1rem', margin: '2rem' }}>CREATE NEW ORDER</Button>
                        </Col>
                    </Row>
