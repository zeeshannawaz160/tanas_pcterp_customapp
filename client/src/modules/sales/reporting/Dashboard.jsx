...
export default function Dashboard() {
    ...
    const handleSelectOption = async (name, duration) => {
        ...
        const response = await ApiService.get(`salesOrder/salesAnalysis?name=${name}&start=${startTime}&end=${endTime}`);
                    ...
                    <Row>
                        <Col>
                            <Form.Select className='reporting-select' id="reportName" name="reportName" aria-label="Default select example" onChange={e => {
                                ...
                                <option className='reporting-select-option' value="Quantity Invoiced">Quantity Invoiced</option>
                                <option className='reporting-select-option' value="Quantity Delivered">Quantity Delivered</option>
                            </Form.Select>
                        ...
                        <Row>
                            <Col md="3" sm="6" xs="6">
                                <div style={{ margin: 10, borderLeft: '2px solid #e4e4e4', paddingLeft: 10 }}>
                                    <h6 style={{ color: 'rgb(103,103,103)', fontWeight: '400' }}>Total Sales</h6>
                                    <h4 style={{ color: '#444B5A' }}>{salesOrder.total ? formatNumber((salesOrder.total)?.toFixed(2)) : (0).toFixed(2)}</h4>
                                </div>
                            </Col>
                            <Col md="3" sm="6" xs="6">
                                <div style={{ margin: 10, borderLeft: '2px solid #e4e4e4', paddingLeft: 10 }}>
                                    <h6 style={{ color: 'rgb(103,103,103)', fontWeight: '400' }}>Untaxed Total</h6>
                                    <h4 style={{ color: '#444B5A' }}>{salesOrder.untaxedAmount ? formatNumber((salesOrder.untaxedAmount)?.toFixed(2)) : (0).toFixed(2)}</h4>
                                </div>
                            </Col>
                            <Col md="3" sm="6" xs="6">
                                <div style={{ margin: 10, borderLeft: '2px solid #e4e4e4', paddingLeft: 10 }}>
                                    <h6 style={{ color: 'rgb(103,103,103)', fontWeight: '400' }}>Orders</h6>
                                    <h4 style={{ color: '#444B5A' }}>{salesOrder.noOfOrders ? salesOrder.noOfOrders : 0}</h4>
                                </div>
                            </Col>
                            <Col md="3" sm="6" xs="6">
                                <div style={{ margin: 10, borderLeft: '2px solid #e4e4e4', paddingLeft: 10 }}>
                                    <h6 style={{ color: 'rgb(103,103,103)', fontWeight: '400' }}>Average Order</h6>
                                    <h4 style={{ color: '#444B5A' }}>{(salesOrder.total / salesOrder.noOfOrders) ? formatNumber((salesOrder.total / salesOrder.noOfOrders)?.toFixed(2)) : (0).toFixed(2)}</h4>
                                </div>
                            </Col>
                        </Row>
                    </Container>
