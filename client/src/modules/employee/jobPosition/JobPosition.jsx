...
export default function JobPosition() {
    ...
    const createDocument = (data) => {
        ...
        return ApiService.post('/jobPosition', data).then(response => {
            if (response.data.isSuccess) {
                history.push("/employees/jobpositions");
            }
        }).catch(e => {
    ...
    useEffect(async () => {
        try {
            ...
            if (!isAddMode) {
                ApiService.setHeader();
                const response = await ApiService.get(`jobPosition/${id}`)
                // .then(response => {
                ...
                <Container className="pct-app-content-header  m-0 pb-2" style={{ borderBottom: '1px solid black' }} fluid>
                    <Row>
                        <Breadcrumb style={{ fontSize: '24px' }}>
                            <Breadcrumb.Item className="breadcrumb-item" linkAs={Link} linkProps={{ to: '/employees/jobpositions' }} ><h3 className="breadcrum-label">Job Positions</h3></Breadcrumb.Item>
                    ...
                    <Row>
                        <Col>
                            ...
                            <Button as={Link} to="/employees/jobpositions" variant="light" size="sm">DISCARD</Button>
                ...
                <Container className="pct-app-content-body p-0 m-0 mt-2" fluid>
                    <Container className="mt-2" fluid>
                        <Row>
                            ...
                            <Form.Group as={Col} md="4" className="mb-2">
                                <Form.Label className="m-0">Job Description</Form.Label>
                                <Form.Control
                                    as="textarea"
                                    id="jobDescription"
                                    name="jobDescription"
