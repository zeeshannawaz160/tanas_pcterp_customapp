...
export default function CustomerList() {
    ...
    const columns = [
        {
            headerName: '#', field: 'id', sortable: false, filter: false, cellRendererFramework: (params) =>
                <Button style={{ minWidth: "4rem" }} size="sm" as={Link} to={`/pos/customer/${params.value}?mode=edit&stack=${stack}`}><BsBoxArrowInUpRight /></Button>
        },
    ...
    const handleSetCustomer = (currentCustomer) => {
        ...
        if (stack === 'dashboard') history.push('/pos');
        if (stack === 'payment') history.push('/pos/payment');
        if (stack === 'refund') history.push('/pos/refund');
        // history.push("/pos");
        // history.goBack();
    }
